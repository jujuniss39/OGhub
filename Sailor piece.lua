...
if not Plr then
    error("LocalPlayer not found! Script may not be running in a game.")
end
...
local function GetServiceNPC(name)
    local path = workspace:FindFirstChild('ServiceNPCs')
...
if RS and RS:FindFirstChild("Modules") then
    ...
    Modules.Quests = SafeRequire(mods:FindFirstChild("QuestConfig"))
    Modules.Trait = SafeRequire(mods:FindFirstChild("TraitConfig"))
    Modules.Race = SafeRequire(mods:FindFirstChild("RaceConfig"))
    Modules.Clan = SafeRequire(mods:FindFirstChild("ClanConfig"))
    Modules.Stats = SafeRequire(mods:FindFirstChild("StatRerollConfig"))
    Modules.ItemRarity = SafeRequire(mods:FindFirstChild("ItemRarityConfig"))
    Modules.Title = SafeRequire(mods:FindFirstChild("TitlesConfig"))
    Modules.SpecPassive = SafeRequire(mods:FindFirstChild("SpecPassiveConfig"))
end

-- ========== UPDATE FUNCTIONS ==========
local function UpdateNPCLists()
    local path = workspace:FindFirstChild('NPCs')
...
-- ========== POPULATE LISTS ==========
if Modules.Stats and Modules.Stats.StatKeys then
...
local function ExecuteFarmLogic(target)
    ...
    if tick() - Shared.LastM1 >= m1Delay then
        pcall(function()
            if Remotes.M1 then Remotes.M1:FireServer() end
        end)
        Shared.LastM1 = tick()
    end
end
...
task.spawn(function()
    while getgenv().taolao_Running do
        ...
        if Toggles.AutoTrait.Value then
            ...
            if traitUI then
                local mainFrame = traitUI:FindFirstChild("MainFrame", true)
                if mainFrame then
                    local currentTrait = mainFrame:FindFirstChild("TraitGotten", true)
        ...
        if Toggles.AutoRace.Value then
            ...
            if currentRace and selected[currentRace] then
        ...
        if Toggles.AutoClan.Value then
            ...
            if currentClan and selected[currentClan] then
